...
interface HeroShotProps {
    ...
    alt: string;
    /**
     * `true` (domyślnie) = zdjęcie ładujemy natychmiast z `fetchPriority="high"`.
     * Ustaw `false`, jeśli kadr kiedyś wyląduje poniżej zgięcia strony.
     */
    eager?: boolean;
}
...
/**
 * Geometryczny odpowiednik dawnego `<image x="70" y="52" width="460" height="345">`
 * w układzie SVG 600×450, przeliczony na procenty, żeby kadr był responsywny:
 *
 *   left/right = 70 / 600          = 11,6667 %
 *   top        = 52 / 450          = 11,5556 %
 *   bottom     = (450−52−345) / 450 = 11,7778 %
 *
 * Dzięki temu zdjęcie leży dokładnie tam, gdzie rysowało je SVG.
 */
const PHOTO_BOX_STYLE: React.CSSProperties = {
    top: "11.5556%",
    right: "11.6667%",
    bottom: "11.7778%",
    left: "11.6667%",
...
/**
 * Szerokość zdjęcia w CSS-owych pikselach dla `srcset`/`sizes`.
 *
 * Kadr zajmuje 76,67 % szerokości kontenera (`max-w-lg` = 512 px,
 * `xl:max-w-xl` = 576 px), więc:
 *   • telefon  (< 640 px):  76,67 % × (100vw − 2 × px-4)  ≈ 77vw − 1,6rem
 *   • 640–1279 px:          76,67 % × 512 px             ≈ 393 px
 *   • ≥ 1280 px (xl):       76,67 % × 576 px             ≈ 442 px
 *
 * Bez `sizes` next/image strzelałby z szerokością i serwował albo za duży,
 * albo rozmyty wariant.
 */
const HERO_PHOTO_SIZES =
    "(max-width: 639px) calc(77vw - 1.6rem), (max-width: 1279px) 393px, 442px";

/** Zastępczy kadr, gdy żadne zdjęcie się nie załadowało (brak pliku w storage). */
function HeroPhotoPlaceholder() {
    return (
        <span className="flex h-full w-full flex-col items-center justify-center gap-3 text-neutral-400">
            <Zap className="h-8 w-8" strokeWidth={1.75} aria-hidden="true" />
 ...
 *
 * ── Dlaczego zdjęcie jest teraz zwykłym `<img>`, a nie `<image>` w SVG ──
 *
 * Poprzednia wersja rysowała produkt jako `<image href="…">` wewnątrz SVG.
 * To wyglądało tak samo, ale kosztowało bardzo dużo:
 *
 *   1. SVG `<image>` nie przechodzi przez optimizer `next/image`, więc do
 *      przeglądarki leciał oryginalny plik ze storage (pełny JPEG, setki KiB).
 *   2. Żądania obrazów z SVG przeglądarka odkrywa dopiero przy renderowaniu
 *      drzewa SVG — nie da się im ustawić ani `fetchpriority`, ani `loading`,
 *      ani `srcset`. Lighthouse raportował to jako „Wykrywanie żądań LCP"
 *      (opóźnienie ładowania zasobu ~1030 ms) i jako element LCP, którego
 *      „nie można znaleźć w dokumencie początkowym".
 *
 * Teraz kadr to trzy warstwy w tej samej ramce `relative aspect-[4/3]`:
 *
 *   1. SVG (spód)   — biała „karta" z maską i miękkim zaniknięciem,
 *   2. `<img>`      — prawdziwy `next/image` (AVIF/WebP, `srcset`, `sizes`,
 *                     `loading="eager"`, `fetchPriority="high"`),
 *   3. SVG (wierzch)— poszarpany zielony border.
 *
 * Kolejność ma znaczenie: filtr `feDisplacementMap` przesuwa obrys nawet o ~7 px,
 * a margines zdjęcia do krawędzi kadru to tylko ~10 px w pionie, więc border musi
 * być rysowany NAD zdjęciem (w SVG był ostatni — tu jest ostatnią warstwą).
 *
 * Zdjęcie nie potrzebuje już maski: jego ramka (patrz `PHOTO_BOX_STYLE`) leży
 * w całości wewnątrz poszarpanego kształtu, a `object-contain` odpowiada
 * dawnemu `preserveAspectRatio="xMidYMid meet"`.
 *
 * ── Co zostaje z poprzednich ustaleń (Safari!) ──
 *
 *   • Filtry SVG NIGDY nie są wewnątrz `<mask>` (Safari je ignoruje) — maska
 *     zawiera wyłącznie białą ścieżkę, a rozmycie (`feGaussianBlur`) działa
 *     na zewnątrz, na samym prostokącie karty.
 *   • Filtr nie jest nakładany na grupę z obrazkiem — to znane pole minowe
 *     w WebKit (zdjęcie potrafi się w ogóle nie narysować). Teraz obrazka
 *     w SVG nie ma wcale, więc problem znika definitywnie.
 *   • Każdy z dwóch SVG ma WŁASNE `<defs>` (maska/gradient karty w dolnym,
 *     filtr/gradient obrysu w górnym) — brak współdzielonych referencji
 *     między drzewami SVG.
 */
export default function HeroShot({ srcs, alt, eager = true }: HeroShotProps) {
    return (
        <>
            {/* 1/3 — biała karta pod zdjęciem (maska + miękkie zaniknięcie) */}
            <svg
                viewBox="0 0 600 450"
                aria-hidden="true"
                focusable="false"
            ...
            >
                <defs>
                    {/* Miękkie zaniknięcie w tło (fade out) — rozmycie SAMEGO
                        ...
                        Filtr działa wyłącznie na prostokącie karty. */}
                    <filter
                    ...
                    <radialGradient id="hero-card" cx="50%" cy="30%" r="100%">
                        {/* Kolory z `.hero-shot` w globals.css (z zapasem, gdyby
                            komponent został użyty poza tym kontenerem). */}
                        <stop
                            offset="0%"
                            style={{ stopColor: "var(--hero-card-inner, #ffffff)" }}
                        />
                        <stop
                            offset="55%"
                            style={{ stopColor: "var(--hero-card-inner, #ffffff)" }}
                        ...
                        <stop
                            offset="100%"
                            style={{ stopColor: "var(--hero-card-outer, #f5f5f5)" }}
                        />
                    </radialGradient>

                    {/* Maska kadru: sam biały kształt, BEZ FILTRÓW.
                        Filtry w <mask> są nieprzewidywalne w Safari. */}
                    <mask id="hero-frame">
            ...
            {/* 2/3 — zdjęcie produktu: `next/image` (AVIF/WebP + srcset + sizes).
            ...
            >
                <FallbackImage
                    ...
                    fallback={<HeroPhotoPlaceholder />}
                />
            </div>

            {/* 3/3 — nierówny border rysowany po tej samej krawędzi, NAD zdjęciem.
