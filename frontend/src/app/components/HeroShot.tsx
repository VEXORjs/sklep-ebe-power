...
const RAGGED_MASK_URL = `url("data:image/svg+xml,%3Csvg viewBox='0 0 600 450' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='${RAGGED_FRAME_PATH}' fill='black'/%3E%3C/svg%3E")`;
...
export default function HeroShot({ srcs, alt, eager = true }: HeroShotProps) {
    return (
        <>
            {/* 1/3 — biała karta pod zdjęciem (gradient bazy) */}
            <svg
                viewBox="0 0 600 450"
                aria-hidden="true"
                focusable="false"
            ...
            >
                <defs>
                    ...
                    <radialGradient id="hero-card" cx="50%" cy="30%" r="100%">
                        <stop offset="0%" style={{ stopColor: "var(--hero-card-inner, #ffffff)" }} />
                        <stop offset="55%" style={{ stopColor: "var(--hero-card-inner, #ffffff)" }} />
                        <stop offset="100%" style={{ stopColor: "var(--hero-card-outer, #f5f5f5)" }} />
                    </radialGradient>
            ...
            {/* 2/3 — ZDJĘCIE (Obcięte idealnie do maski) */}
            {/* Ten div pokrywa cały rozmiar 600x450 i działa jako nożyczki ucinające wszystko co wystaje */}
            <div
                className="pointer-events-none absolute inset-0 h-full w-full"
                style={{
                    maskImage: RAGGED_MASK_URL,
                    WebkitMaskImage: RAGGED_MASK_URL,
                    maskSize: "100% 100%",
                    maskRepeat: "no-repeat",
                }}
            >
                {/* A wewnątrz pozycjonujemy zdjęcie. `mix-blend-multiply` zostawiamy,
                    żeby to, co znajdzie się wewnątrz maski ładnie wtopiło się w tło */}
                <div
            ...
            {/* 3/3 — nierówny border rysowany NAD zdjęciem, żeby ładnie przykryć linię cięcia */}
            <svg
                viewBox="0 0 600 450"
                aria-hidden="true"
                focusable="false"
